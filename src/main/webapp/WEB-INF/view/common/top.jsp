			...
			<c:choose>
				...
				<c:when test="${sessionScope.user!= null}">
					<li class="nav-item"><a class="nav-link" href="/my/home">
							<img alt="" src="/pic/${sessionScope.user.nickname }"
						...
						<div class="dropdown" style="padding-top: 0.4rem;">
							<a href="#" class="nav-link dropdown-toggle" role="button"
								...
								aria-haspopup="true" aria-expanded="false"> <c:out
									value="${sessionScope.user.username}" default="CMS-User" />
							</a>
							<div class="dropdown-menu dropdown-menu-right"
								...
								<a class="dropdown-item" href="/login/zhuxiao">退出</a>
							</div>
						</div>
					</li>
				</c:when>
				<c:otherwise>
					<%-- 未登录显示登录注册链接 --%>
					<li class="nav-item"><a class="nav-link" href="/login/reg">注册</a></li>
					<li class="nav-item"><a class="nav-link" href="/login/gologin">登录</a></li>
				</c:otherwise>
			</c:choose>
		</ul>
	</nav>
